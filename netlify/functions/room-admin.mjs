import {deleteMedia} from './lib/media-store.mjs';
...
export default async req=>{
 ...
 if(b.action==='reset'){
   if(b.area==='album'){const {blobs}=await store.list({prefix:'meta/'});for(const x of blobs){const m=await store.get(x.key,{type:'json'});if(m?.mediaId)await deleteMedia(m.mediaId);}}
 ...
 if(b.action==='delete'&&b.area==='album'&&/^[a-f0-9-]{36}$/.test(b.id||'')){const m=await store.get('meta/'+b.id,{type:'json'});if(m?.mediaId)await deleteMedia(m.mediaId);await store.delete('meta/'+b.id);await store.delete('image/'+b.id);return reply({ok:true})}
