(async function(){
  ...
  function renderGuest(){
    $('#dashAvatar').innerHTML='<div class="guest-avatar-v31">🧭</div>';
    ...
    primary.href='select-student.html?force=1';
    primary.querySelector('b').textContent='학생으로 시작하기';
    primary.onclick=()=>SJ.setRole('student');
  }

  function renderStudent(){
    p=SJ.current();st=SJGear.normalizeState(SJ.load());
    ...
    const primary=$('#settingsPrimary');
    primary.href='equipment.html';
    primary.querySelector('b').textContent='내 캐릭터 꾸미기';
  ...
  // First visit: show the real home screen first, then ask only once.
  if(!role){
    ...
    try{
      ...
      if(reg.registered){
        ...
        // This path is mainly for a password reset by the teacher.
        await setupPassword();
      }
