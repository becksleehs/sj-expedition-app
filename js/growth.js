(function(){
  ...
  function normalizeState(s){
    s=s||SJ.load();s.xp=Math.max(0,Number(s.xp)||0);s.growthLevel=clamp(Number(s.growthLevel)||1,1,5);s.avatarLevel=clamp(Number(s.avatarLevel)||s.growthLevel,1,s.growthLevel);
  ...
  async function checkUpgradePrompt(force=false){if(!/(?:^|\/)(?:index\.html)?$/.test(location.pathname))return false;let s=normalizeState(SJ.load());if(SJ.getRole&&SJ.getRole()!=='student')return false;if(!s.studentId||!canUpgrade(s))return false;const target=s.growthLevel+1;if(prompted.has(target))return false;prompted.add(target);const m=ensureModal(),p=SJ.current(),n=info(target);render(m.querySelector('#growthUpgradePreview'),s.studentId,target);m.querySelector('#growthUpgradeText').innerHTML=`<b>${p?.name||''}</b> 학생이 <strong>Lv.${target} ${n.name}</strong>로 성장할 수 있어요!<br><small>${n.desc}</small>`;m.hidden=false;s.lastUpgradePromptLevel=target;SJ.save(s);m.querySelector('#growthUpgradeNow').onclick=async()=>{m.querySelector('#growthUpgradeNow').disabled=true;try{await upgrade();m.hidden=true;location.reload()}catch(e){alert(e.message);m.querySelector('#growthUpgradeNow').disabled=false}};m.querySelector('#growthUpgradeLater').onclick=()=>m.hidden=true;return true}
