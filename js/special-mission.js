(function(){
  ...
  function ensureModal(){
    ...
    wrap.innerHTML=`<div class="special-card-v34"><div class="special-badge-v34">🎁 EXPEDITION NEWS</div><h2>돌발 안내 도착!</h2><div id="specialMissionPopupText" class="special-text-v34"></div><button id="specialMissionConfirm" type="button">확인했어요</button><button id="specialMissionNotify" class="special-notify-v34" type="button">🔔 휴대폰 알림 켜기</button><small>선생님이 새 돌발 안내을 보내면 이 창으로 알려드려요.</small></div>`;
    document.body.appendChild(wrap);
    wrap.querySelector('#specialMissionConfirm').onclick=()=>{if(lastId)localStorage.setItem(SEEN,lastId);wrap.hidden=true};
    wrap.querySelector('#specialMissionNotify').onclick=async()=>{if(!('Notification'in window)){alert('이 브라우저는 알림 기능을 지원하지 않습니다.');return}const p=await Notification.requestPermission();if(p==='granted'){wrap.querySelector('#specialMissionNotify').textContent='🔔 휴대폰 알림 켜짐';try{const reg=await navigator.serviceWorker.ready;await reg.showNotification('승주 원정대',{body:'돌발 안내 알림이 켜졌어요!',tag:'sj-special-ready'});}catch{}}};
    return wrap;
  }
  async function systemNotify(mission){
    if(!('Notification'in window)||Notification.permission!=='granted')return;
    try{const reg=await navigator.serviceWorker.ready;await reg.showNotification('🎁 승주 원정대 돌발 안내',{body:mission.text,tag:`sj-special-${mission.id}`,renotify:true,data:{url:'index.html'}});}catch{}
  }
  function updateCard(mission){if(document.getElementById('missionBtn'))return;
    ...
    if(!text)return;
    if(mission){text.innerHTML=esc(mission.text).replace(/\n/g,'<br>');sub.textContent='새 돌발 안내이 도착했어요!';paper?.classList.add('open');}
    else{text.innerHTML='돌발 안내이<br>아직 공개되지 않았어요!';sub.textContent='선생님이 보내면 팝업으로 바로 알려드려요.';paper?.classList.remove('open');}
  }
